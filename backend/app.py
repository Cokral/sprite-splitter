...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # For development, use ["http://localhost:YOUR_FRONTEND_PORT"] in production
...
def split_image(image_bytes, rows, cols, original_filename, user_id):
    ...
    for i in range(rows):
        for j in range(cols):
            box = (j * sprite_width, i * sprite_height, 
                   (j + 1) * sprite_width, (i + 1) * sprite_height)
            sprite = img.crop(box)
...
async def download_sprites(x_user_id: str = Header(...)):
    ...
    # Return the zip file
    return FileResponse(temp_zip.name, media_type="application/zip", filename="split_images.zip")
